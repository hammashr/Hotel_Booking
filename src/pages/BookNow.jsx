...
const BookNow = () => {
  ...
  const CANCELLATION_POLICY =
    "100% refund up to 30 days before arrival, 50% refund up to 14 days before arrival. No refunds for cancellations made within 14 days of check-in.";
  ...
  const resolvedPackageCode = useMemo(() => {
    ...
    if (
      explicitCode &&
      ["standard", "signature", "extended"].includes(explicitCode)
    ) {
      return explicitCode;
    }
    ...
    const syncPackageMeta = async () => {
      ...
      try {
        ...
        const matchedPackage = packages.find(
          (pkg) => pkg.code === resolvedPackageCode,
        );
        ...
        setCalendarMeta({
          ...
          pricePerNight:
            Number(matchedPackage?.pricePerNight) ||
        ...
        showToast(
          "error",
          "Could not load live package rates. Using fallback values for now.",
        );
      }
    };
  ...
  const nightSplit = countWeekdayWeekendNights(
    bookingData.checkIn,
    bookingData.checkOut,
  );
  // Prefer live pricing preview from server when available
  const estimatedSubtotal = livePricing
    ? livePricing.subtotal
    : Number((calendarMeta.pricePerNight * selectedNights).toFixed(2));
  const selectedAddOnTotal = livePricing
    ? livePricing.addOnsFee
    : Number(
        bookingData.addOns
          .reduce((sum, option) => sum + (ADD_ON_PRICE_MAP[option] || 0), 0)
          .toFixed(2),
      );
  const estimatedCleaningFee = livePricing
    ? livePricing.cleaningFee
    : CLEANING_FEE;
  const estimatedTax = livePricing
    ? livePricing.tax
    : Number(
        (
          (estimatedSubtotal + selectedAddOnTotal + estimatedCleaningFee) *
          TAX_RATE
        ).toFixed(2),
      );
  ...
  const nightlyBreakdownLabel = useMemo(() => {
    const weekdayNights =
      livePricing?.weekdayNights ?? nightSplit.weekdayNights;
    const weekendNights =
      livePricing?.weekendNights ?? nightSplit.weekendNights;
    ...
    const parts = [];
    if (weekdayNights > 0)
      parts.push(`${weekdayNights} weekday × $${ratePlan.weekday}`);
    if (weekendNights > 0)
      parts.push(`${weekendNights} weekend × $${ratePlan.weekend}`);

    return parts.length ? parts.join(" + ") : `0 night(s)`;
  }, [
    livePricing?.weekdayNights,
    livePricing?.weekendNights,
    nightSplit.weekdayNights,
    nightSplit.weekendNights,
    ratePlan,
    calendarMeta.pricePerNight,
    selectedNights,
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(
        "error",
        error?.message || "Booking request failed. Please try again.",
      );
      return;
    }
      ...
      getFAQSchema([
        {
          ...
          answer:
            "Most requests receive a response within 24 hours after availability review.",
        },
        {
          question: "Is payment required immediately?",
          answer:
            "No. Availability and final pricing are confirmed first before any payment steps.",
        },
      ]),
    ],
    [],
        ...
        <div
          className={`min-h-screen transition-colors duration-500 ${
            isDarkMode
              ...
              : "bg-linear-to-b from-white to-[#F8FAFB] text-[#0F172A]"
          }`}
        >
          <div className="container mx-auto px-4 py-16">
            <div className="max-w-2xl mx-auto text-center">
              <div className="mb-8">
                ...
                <h1
                  className={`text-4xl font-bold mb-4 ${isDarkMode ? "text-[#E0E7EE]" : "text-[#0F172A]"}`}
                >
                  Request Received!
                </h1>
                <p
                  className={`text-xl mb-6 ${isDarkMode ? "text-[#C4CCD4]" : "text-[#374151]"}`}
                >
              ...
              <div
                className={`p-8 rounded-xl ${isDarkMode ? "bg-[#141A1F]" : "bg-white border border-[#E2E8F0]"} ${
                  isDarkMode
              ...
              >
                <h2
                  className={`text-2xl font-bold mb-4 ${isDarkMode ? "text-[#E0E7EE]" : "text-[#0F172A]"}`}
                >
                  What Happens Next
                </h2>
                <div
                  className={`text-left space-y-4 ${isDarkMode ? "text-[#C4CCD4]" : "text-[#374151]"}`}
                >
                  <div className="flex items-start gap-3">
                    <FaCheckCircle className="text-green-500 mt-1 shrink-0" />
                    <p>
                      We review availability and match your dates with the best
                      stay
                    </p>
                  </div>
                  <div className="flex items-start gap-3">
                    <FaCheckCircle className="text-green-500 mt-1 shrink-0" />
                    <p>
                      You will receive a reply within 24 hours with options and
                      pricing
                    </p>
                  </div>
                  <div className="flex items-start gap-3">
                    <FaCheckCircle className="text-green-500 mt-1 shrink-0" />
                    <p>
                      We confirm your stay once you approve the final details
                    </p>
                  </div>
                  <div className="flex items-start gap-3">
                    <FaCheckCircle className="text-green-500 mt-1 shrink-0" />
                    <p>
                      Check-in instructions are sent 48 hours before arrival
                    </p>
                  </div>
                </div>

                <div
                  className={`mt-6 p-4 rounded-lg ${isDarkMode ? "bg-[#0F1419]" : "bg-[#F8FAFC]"} `}
                >
                  <p
                    className={`font-semibold mb-2 ${isDarkMode ? "text-[#E0E7EE]" : "text-[#0F172A]"}`}
                  >
              ...
              {PAYMENTS_ENABLED && !depositPaid && (
                ...
                >
                  <h2
                    className={`text-xl font-bold mb-1 ${isDarkMode ? "text-[#E0E7EE]" : "text-[#0F172A]"}`}
                  >
                    Secure your booking with a deposit
                  </h2>
                  <p
                    className={`text-sm mb-5 ${isDarkMode ? "text-[#8A9BAC]" : "text-[#64748B]"}`}
                  >
                    A refundable deposit holds your dates while we confirm
                    availability.
                  </p>
                  {depositError && (
              ...
              {depositPaid && (
                <div
                  className={`flex items-center gap-3 p-5 rounded-xl mb-8 ${
                    isDarkMode ? "bg-[#1A2E1A]" : "bg-[#EAF3EA]"
                  }`}
                >
                  <FaCheckCircle className="text-[#2F5D3A] h-5 w-5 shrink-0" />
                  <p
                    className={`text-sm font-semibold ${isDarkMode ? "text-[#6BAF7A]" : "text-[#1F3A2A]"}`}
                  >
  ...
  return (
    <PageLayout
      seo={{
        ...
        description:
          "Request availability for Tiny Escape stays in Texas Hill Country. Clear pricing, fast confirmation, and a secure booking inquiry flow.",
        keywords:
          "Tiny Escape booking, stay availability, Texas Hill Country lodging, tiny home reservation request",
        url: "/book-now",
        structuredData: bookingStructuredData,
      }}
    >
      ...
      {/* Hero */}
      <section
        className={`relative py-12 border-b ${isDarkMode ? "bg-linear-to-br from-[#0F0D0A] via-[#171310] to-[#0F0D0A] border-[#2A2119]" : "bg-linear-to-br from-[#FFF9F1] via-[#F5F0E6] to-[#FFF9F1] border-[#E6D7C3]"}`}
      >
      ...
      {/* Progress Bar */}
      <div
        className={`py-4 backdrop-blur-lg border-b ${isDarkMode ? "bg-[#14110E]/95 border-[#2A2119]" : "bg-[#FFFCF7]/95 border-[#E6D7C3]"}`}
      >
        <div className="container mx-auto px-4">
          <div
            className={`text-center text-sm mb-3 font-semibold ${mutedTextClass}`}
          >
            Step {step} of {progressSteps.length} •{" "}
            {progressSteps.find((item) => item.number === step)?.title}
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              <form onSubmit={step === 3 ? handleSubmit : handleNextStep}>
                <div
                  className={`p-8 rounded-2xl ${isDarkMode ? "bg-[#141A1F]" : "bg-white border border-[#E2E8F0]"} ${
                    isDarkMode
                      ? "shadow-[0_10px_24px_-18px_rgba(0,0,0,0.7)]"
                      : "shadow-[0_10px_24px_-18px_rgba(15,23,42,0.18)]"
                  }`}
                >
                  ...
                  {step === 1 && (
                    <div className="space-y-6">
                      <h2
                        className={`text-2xl font-bold mb-6 ${sectionTitleClass}`}
                      >
                        Guest Information
                      </h2>
                      ...
                      <div>
                        ...
                        {!houseSlug && (
                          <p
                            className={`text-sm mt-2 ${isDarkMode ? "text-amber-400" : "text-amber-600"}`}
                          >
                            House slug is missing in this request context. Date
                            blocking may be limited.
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  {step === 2 && (
                    <div className="space-y-6">
                      ...
                      <div>
                        <label
                          className={`block mb-3 font-semibold ${labelClass}`}
                        >
                          Add-ons
                        </label>
                        <div className="grid md:grid-cols-2 gap-4">
                          {ADD_ON_OPTIONS.map((option) => (
                            ...
                            >
                              ...
                              />
                              <span
                                className={`font-semibold ${isDarkMode ? "text-[#E0E7EE]" : "text-[#0F172A]"}`}
                              >
                                {option.label}
                              </span>
                              <span
                                className={`ml-2 text-sm font-medium ${isDarkMode ? "text-[#C9A36A]" : "text-[#2F5D3A]"}`}
                              >
                                +${option.price}
                              </span>
                            </label>
                  ...
                  {step === 3 && (
                    <div className="space-y-6">
                      <h2
                        className={`text-2xl font-bold mb-6 ${isDarkMode ? "text-[#E0E7EE]" : "text-[#0F172A]"}`}
                      >
                        Review and Submit
                      </h2>
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          />
                          <div
                            className={`text-sm ${isDarkMode ? "text-[#E8F5E9]" : "text-[#1F3A29]"}`}
                          >
                            <p className="font-semibold mb-1">
                              Request Review:
                            </p>
                            <p>
                              We will confirm availability and send final
                              details before any payment is required.
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid md:grid-cols-2 gap-4">
                        <div
                          className={`p-4 rounded-lg ${isDarkMode ? "bg-[#0F1419]" : "bg-[#F8FAFC]"} `}
                        >
                          <div
                            className={`text-xs uppercase tracking-wider ${isDarkMode ? "text-[#8B949E]" : "text-[#64748B]"}`}
                          >
                            Dates
                          </div>
                          <div
                            className={`font-semibold ${isDarkMode ? "text-[#E0E7EE]" : "text-[#0F172A]"}`}
                          >
                            {bookingData.checkIn || "TBD"} to{" "}
                            {bookingData.checkOut || "TBD"}
                          </div>
                        </div>
                        <div
                          className={`p-4 rounded-lg ${isDarkMode ? "bg-[#0F1419]" : "bg-[#F8FAFC]"} `}
                      ...
                      {bookingData.addOns.length > 0 && (
                        ...
                        >
                          <div
                            className={`text-xs uppercase tracking-wider mb-2 ${isDarkMode ? "text-[#8B949E]" : "text-[#64748B]"}`}
                          >
                            Selected Add-ons (Applied to Checkout)
                          </div>
                          <div className="space-y-2">
                            {bookingData.addOns.map((addOn) => {
                              const option = ADD_ON_OPTIONS.find(
                                (item) => item.value === addOn,
                              );
                      ...
                      <div
                        className={`p-4 rounded-lg border ${isDarkMode ? "bg-[#0F1419] border-[#1E242B]" : "bg-[#F8FAFC] border-[#E2E8F0]"}`}
                      >
                        <div
                          className={`text-xs uppercase tracking-wider mb-3 ${isDarkMode ? "text-[#8B949E]" : "text-[#64748B]"}`}
                        >
                          Checkout Breakdown
                        </div>
                        <div className="space-y-2 text-sm">
                          <div className="flex items-center justify-between">
                            <span
                              className={
                                isDarkMode ? "text-[#C4CCD4]" : "text-[#475569]"
                              }
                            >
                          ...
                          <div
                            className={`pt-2 mt-2 border-t flex items-center justify-between font-bold ${isDarkMode ? "border-[#1E242B] text-[#E0E7EE]" : "border-[#E2E8F0] text-[#0F172A]"}`}
                          >
                      ...
                      >
                        ...
                        <div
                          className={`text-sm space-y-1 ${isDarkMode ? "text-[#C4CCD4]" : "text-[#475569]"}`}
                        >
                          {ratePlan && (
                            <p>
                              {ratePlan.label}: Weekday ${ratePlan.weekday},
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        />
                        <label
                          className={`text-sm ${isDarkMode ? "text-[#C4CCD4]" : "text-[#475569]"}`}
                        >
                          I agree to the{" "}
                          <a
                  ...
                  {/* Navigation Buttons */}
                  <div
                    className={`sticky bottom-0 z-20 -mx-8 mt-8 px-8 py-4 border-t ${
                      isDarkMode
                        ? "border-[#2A2119] bg-[#14110E]/95"
                        : "border-[#E6D7C3] bg-[#FFFCF7]/95"
                    } backdrop-blur-lg`}
                  >
                  ...
                  {step === 1 && !canContinueStep1 && (
                    <p
                      className={`mt-3 text-sm ${isDarkMode ? "text-amber-400" : "text-amber-700"}`}
                    >
                      Select a valid available date range that meets minimum
                      nights to continue.
                    </p>
                  )}
                  {submissionState.error && (
                    <p
                      className={`mt-3 text-sm ${isDarkMode ? "text-red-400" : "text-red-600"}`}
                    >
                      {submissionState.error}
                    </p>
                  )}
            ...
            <div>
              <div
                className={`p-6 rounded-xl ${isDarkMode ? "bg-[#141A1F]" : "bg-white border border-[#E2E8F0]"} ${
                  isDarkMode
                    ? "shadow-[0_10px_24px_-18px_rgba(0,0,0,0.7)]"
                    : "shadow-[0_10px_24px_-18px_rgba(15,23,42,0.18)]"
                }`}
              >
                <h3
                  className={`text-xl font-bold mb-4 ${isDarkMode ? "text-[#E0E7EE]" : "text-[#0F172A]"}`}
                >
                  Request Summary
                </h3>
                ...
                <div className="space-y-3 mb-6">
                  <div
                    className={`text-sm ${isDarkMode ? "text-[#8B949E]" : "text-[#475569]"}`}
                  >
                    Stay
                  </div>
                  <div
                  ...
                  <div
                    className={`text-sm ${isDarkMode ? "text-[#8B949E]" : "text-[#475569]"}`}
                  >
                    Dates: {bookingData.checkIn || "TBD"} to{" "}
                    {bookingData.checkOut || "TBD"}
                  </div>
                  <div
                    className={`text-sm ${isDarkMode ? "text-[#8B949E]" : "text-[#475569]"}`}
                ...
                <div
                  className={`mb-6 rounded-lg border p-4 ${isDarkMode ? "border-[#1E242B] bg-[#0F1419]" : "border-[#E2E8F0] bg-[#F8FAFC]"}`}
                >
                  <h4
                    className={`text-sm font-bold mb-3 ${isDarkMode ? "text-[#E0E7EE]" : "text-[#0F172A]"}`}
                  >
                    Price Breakdown
                  </h4>
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      >
                        ${estimatedCleaningFee.toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span
                    ...
                    <div
                      className={`mt-2 pt-2 border-t flex justify-between font-bold ${isDarkMode ? "border-[#1E242B] text-[#E0E7EE]" : "border-[#E2E8F0] text-[#0F172A]"}`}
                    >
                      <span>Estimated Total</span>
                      <span>${estimatedTotal.toFixed(2)}</span>
                    </div>
                ...
                <div
                  className={`border-t pt-4 space-y-2 ${isDarkMode ? "border-[#1E242B]" : "border-[#E2E8F0]"} `}
                >
                  <div className="flex justify-between">
                    <span
                      className={
                ...
                <div
                  className={`border-t mt-4 pt-4 ${isDarkMode ? "border-[#1E242B]" : "border-[#E2E8F0]"} `}
                >
                  <div
                    className={`text-sm ${isDarkMode ? "text-[#C4CCD4]" : "text-[#475569]"}`}
                  >
                  ...
                  <div
                    className={`text-xs mt-2 ${isDarkMode ? "text-[#A7B0BA]" : "text-[#64748B]"}`}
                  >
                ...
                <div
                  className={`mt-6 p-4 rounded-lg ${isDarkMode ? "bg-green-500/10" : "bg-green-50 border border-green-200"} `}
                >
                  ...
                  <p
                    className={`text-xs ${isDarkMode ? "text-[#C4CCD4]" : "text-[#475569]"}`}
                  >
